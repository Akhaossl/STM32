...
void ParseData_Init(ParseData_Struct *ParseData, uint16_t BufLeng)
{
	...
	ParseData->Buff = (uint8_t*)malloc(sizeof(uint8_t) * BufLeng);//分配BufLeng个uint8_t型存储单元，并将该单元的首地址存储到指针变量ringBuff->Ring_Buff中
...
/*-----------------------------------------------------------
函数功能: 解析环形缓存区的数据
输入参数: 环形缓存区结构体参数，解析数据结构体参数
返 回 值: None
说    明: 从环形缓存区读取数据并传入到“ParseDataFunction()”解析数据函数
进行解析，若解析出一帧完整数据“ParseData->Flag”标志位会自动置1，
解析出的数据缓存在“ParseData->Buff”，然后函数停止解析，
待用户手动将“ParseData->Flag”标志位清0后，函数才能继续往下解析数据。
 -----------------------------------------------------------*/
void ParseData_RingBuff(RingBuf_Struct *RingBuff, ParseData_Struct *ParseData)
{
	RingBuff_Mutex(RingBuff);//环形缓存区互斥访问保护函数
	if(!ParseData->Flag)//判断解析出的数据是否已处理
	{
		uint8_t Data;
		if(RingBuff_Read(RingBuff, &Data))//判断缓存区是否有新数据
		{
			ParseDataFunction(ParseData, Data);//解析数据
		}
	}
}
